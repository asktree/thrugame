...
(function (root) {
  ...
  for (const p of PRODUCTS) {
    ...
    const first = EXAMPLES.find(ex => ex.product === p.key && ex.puzzle.output);
    if (first && first.puzzle.output.elems.includes('Sa') && !p.glyphs.includes('calcifiers')) throw new Error(p.key + ': salt in the product, no calcifier on the tray');
  }
